...
const catalog = [
  ...
  {
    id: "gt3rs-engine", type: "engine", maker: "Porsche", series: "Powertrain lab", name: "GT3 RS Flat-Six",
    tag: "Segmented teardown · 4.0 L · dry sump", description: "Open the GT3 RS engine directly and separate its scanned regions into an exploded technical study.",
    manifest: "./assets/engines/teardown_gt3rs.json", accent: "#f2bd39", power: "518 hp", top: "9,000 rpm", weight: "Dry sump", dbName: "Porsche 911 GT3 RS",
    attribution: "Engine geometry extracted from the supplied GT3 RS assembly",
    info: engineInfo("flat-six", "4.0 L naturally aspirated", "9,000 rpm"),
  },
  {
...
function renderPartRail() {
  ...
  Object.keys(state.asset.info).forEach((key, index) => {
    ...
    button.textContent = `${String(index + 1).padStart(2, "0")} / ${partTitle(key)}`;
...
async function loadAsset(asset) {
  ...
  setExplode(0);
  try {
    const root = asset.manifest ? await loadMultipart(asset, token) : await loadGlb(asset.model, token);
...
function buildPart(object, index, count, asset, center, size) {
  ...
  let category;
  if (asset.type === "engine") category = categorizeEngine(object.name);
  else {
  ...
  const explodeWorld = direction.multiplyScalar(Math.max(size.x, size.z) * (asset.type === "engine" ? 1.0 : .72));
...
function mapTarget(target) {
  ...
  return ({ rear_wing: "wing", brakes: "wheels", suspension: "wheels", transmission: "engine", cooling: "engine", fuel: "engine", front: "hood", rear: "engine", top: "wing", whole: "whole_car", car: "whole_car" })[value] || value;
}
...
async function ask(message) {
  ...
  const verified = response.data && Object.keys(response.data).length > 0;
  addMessage("assistant", response.speech || "Done.", verified ? "Blueprint / Supabase verified" : "Blueprint / scene intelligence");
  state.history.push({ role: "assistant", content: response.speech || "Done." });
}

function getAgentEndpoint() {
  const meta = document.querySelector('meta[name="agent-endpoint"]')?.content;
  if (meta && meta !== "auto") return meta;
  const host = location.hostname;
  if (host.endsWith(".vercel.app")) return "/api/agent";
  if (host === "localhost" || host === "127.0.0.1" || location.port === "4173" || location.port === "5500") {
  ...
  try {
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        context: { car: state.asset.dbName, carId: state.asset.id, selectedPart: state.selected, mode: "learn", assetType: state.asset.type },
      }),
...
function setAgentStatus(connected) {
  elements.agentStatus.innerHTML = connected ? "<i></i> Supabase agent connected" : "<i></i> Local scene knowledge";
}
